        ...
        db.collection("orders").getDocuments() { (snapshot,error) in
            ...
            if error == nil && snapshot != nil {
                ...
                for document in snapshot!.documents {
                    ...
                    self.orders.append(Order(id: data["id"]! as? String, name: data["name"]! as? String, dish: Dish(id: data["dish-id"]! as? String, name: data["dish-name"]! as? String, description: data["dish-description"]! as? String, image: data["dish-image"]! as? String, category: data["dish-category"]! as? String, calories: data["dish-calories"]! as? Int, popular: data["dish-popular"]! as? Int, special: data["dish-special"]! as? Int)))
                }
                //reload data
...
extension ListOrderViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    //xử lý khi click đơn hàng bất kì => dẫn đến trang chi tiết của đơn hàng đó
